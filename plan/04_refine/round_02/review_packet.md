...
**Speed table: every practical public metric for per-step RL
rewards.** The explicit inclusion rule is:

- **Included:** public metrics registered in
  `RadEval/metrics/_registry.py` that can plausibly run in a GRPO
  training loop (sub-second to tens of seconds per 20-sample batch
  on a single GPU).
- **Excluded — by design:**
...
1. **Cached init time (`cached_init_s`) — primary number, reported
   for every metric.** Precondition: models have already been pulled
   into `~/.cache/huggingface` by a one-time `bench_rewards.py
   prewarm` pass that runs before the real measurement. Measurement:
   `t0 = time.perf_counter(); scorer = cls(); t1 = time.perf_counter()`
   — capture with a warm disk cache and no network.
2. **First-download time (`first_download_s`) — anecdotal only,
   captured on a fresh machine.** If the run happens with an empty
   HF cache for that metric, record this as a separate secondary
   column in the JSON but *do not* put it in the main table — the
   doc's table shows `cached_init_s` exclusively so numbers are
   comparable across metrics and reproducible.
3. **Warm per-batch latency** — call
...
- **Default: in-process loop over metrics**, with explicit teardown
  ...
  and the `cached_init_s` measurement taken at `cls()` time only —
  so prior state cannot leak into init timing. No subprocess
  orchestration by default.
- **Optional `--isolate <metric>`** flag spawns a fresh subprocess
  for a single named metric when we observe interference (for
  example, if `radcliq` retains state that inflates the next
  metric's VRAM measurement). This is the YAGNI fallback — we
  introduce it only if we see the problem during implementation.
- **`prewarm` subcommand**: a one-time pass that instantiates each
  scorer once so model weights land in the HF cache; its purpose is
  purely to make `cached_init_s` measurements honest.
- Record `RadEval.__version__`, `torch.__version__`,
  `transformers.__version__`, `trl.__version__`, GPU model,
  Python version, and the HF cache path. One-time at the top of the
  JSON. **No per-metric `cache_hit` heuristic** — `cached_init_s` is
  the reliable number; any "first download" entry is flagged
  explicitly when present.
- Skip metrics whose `cls(**kwargs)` raises `FileNotFoundError` /
  `ImportError` / `HfHubHTTPError` — log to stderr and continue with
  `"skipped": "<reason>"`.
...
- `python scripts/bench_rewards.py prewarm` — instantiate every
  in-scope metric once so HF cache is warm. Optional but recommended
  before the real measurement pass. Writes nothing to disk other
  than HF cache.
- `python scripts/bench_rewards.py all --output docs/benchmarks/trl_rewards_260429.json`
  — generate both E1 (speed) and E2 (divergence) sections in one
  pass and write a single JSON snapshot.
- `python scripts/bench_rewards.py --isolate <metric> --output ...`
  — optional fallback that spawns a fresh subprocess to time one
  named metric, used only if in-process measurement of that metric
  interferes with subsequent metrics.

Uses only stdlib + the installed `RadEval` env. No new deps.

**Default path: in-process loop** — construct each scorer in order,
take its `cached_init_s` and VRAM baseline, run warm-batch timing,
tear down (`del`, `gc.collect`, `torch.cuda.empty_cache()`), move on.
Single JSON emitted at the end.
...
<speed table here — one row per metric, columns: cached init,
warm batch, per-sample ms, peak VRAM>

*`cached init` is the `cls()` time after HF models are already on
disk. First-download time is not reported in this table (see the JSON
snapshot for anecdotal first-download numbers where available).*
...
in the column header. **Gallery is illustrative curated examples, not
a representative sample — it shows disagreement is possible on
plausible inputs, not that it's the common case.**)*

...
- **Predefined rule: `radcliq` runs *last* in the in-process loop.**
  `radcliq` is the only composite metric (BERTScore + SembScore +
  RadGraph stacked), and its footprint is the single biggest risk of
  contamination for adjacent metrics. Running it last means any
  residual state it leaves behind has no subsequent metric to
  contaminate. Simpler than subprocess-merge (drops ~20 LOC, zero
...
**Implementation-time measurement gate** (one-shot validation
during development, before producing the canonical snapshot):

Run one representative **fast** metric (e.g. `bleu`) and one
representative **model-backed** metric (e.g. `bertscore`) in both:

1. The default path (in-process, `radcliq`-last).
2. A fully isolated path (spawn a fresh Python subprocess that
   benchmarks only that one metric via a one-line script).

Compare warm-batch latency and VRAM delta. If in-process matches
isolated within **~10 % latency / ~50 MB VRAM** for both metrics,
proceed with the default in-process plan and record the gate
result as a top-level `"comparability_check"` field in the snapshot
so the doc can cite it. If divergence exceeds the thresholds,
escalate to subprocess-per-model-backed-metric. This ~30-minute
implementation-time check bounds the cross-metric-contamination
concern without expanding scope prematurely.
...
Single-file, ~180-220 LOC. Two commands:
...
- `python scripts/bench_rewards.py --metric <name>` — benchmark a
  single named metric. Used by the measurement gate for in-process
  vs isolated validation, and useful for targeted ad-hoc runs.

Uses only stdlib + the installed `RadEval` env. No new deps.

**Default path: in-process loop** — construct each scorer in a
...
Single JSON emitted at the end. No subprocess orchestration in the
default path.
...
2. `docs(benchmarks): add trl_rewards_260429 snapshot + write-up` —
   the JSON snapshot (including the `comparability_check` gate
   result from implementation time) + `docs/trl_rewards_benchmarks.md`.
   This is where the measured numbers land.
...
- **In-process state leak between metrics.** Mitigation: (a) explicit
  ...
  (b) per-metric VRAM baseline captured immediately before `cls()`,
  (c) the **implementation-time measurement gate** that compares
  in-process vs isolated on one fast + one model-backed metric. If
  the gate passes within ~10% / ~50 MB thresholds, in-process is
  validated; if it fails, escalate to subprocess-per-model-backed-
  metric before producing the canonical snapshot.

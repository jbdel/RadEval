...
1. **Speed table** — cached-init + warm per-batch latency (+
   ...
   slow") into numbers.
2. **Reward divergence gallery** — a hand-picked set of (reference,
   candidate rollout) pairs scored by a few metrics side-by-side,
   illustrating how the **reward signal** a GRPO loop would receive
   changes dramatically with metric choice. Each row is framed as
   "which reward would penalize or reward this rollout?" — making
   metric choice feel like the RL decision it is, not a generic
   evaluation debate. Labelled as illustrative, not representative.
...
**Divergence gallery: a curated subset** — 5 metrics spanning the
lexical → semantic → clinical spectrum:

- `bleu` — rudimentary lexical baseline (NLP default).
- `bertscore` — strong semantic baseline (what a competent NLP
  person reaches for after rejecting BLEU).
...
**Headline baseline: `bertscore`.** BLEU-vs-clinical is a well-worn
strawman; BERTScore-vs-clinical is the stronger story. On a
negation-flip rollout (e.g. "No pleural effusion." → "Pleural
effusion.") BERTScore scores *very* high because most
tokens/embeddings overlap, while F1CheXbert scores 0 because the
*finding* is inverted. That's the screenshot-worthy moment: "even
the strongest drop-in NLP reward would push GRPO toward an
incorrect rollout."

BLEU stays in the gallery as a column so (a) readers comparing
against the obvious lexical reward see the numbers, and (b) the
paraphrase rows — where BLEU ≈ 0 but BERTScore and clinical metrics
all score high — tell a second, complementary story about BLEU
being *also* wrong.

**Note on implementation:** during script development we run both
and confirm the narrative above holds; if BERTScore unexpectedly
agrees with clinical metrics across most rows (e.g. the negation
flip doesn't dramatically favor BERTScore), fall back to
BLEU-as-headline in the prose. The data is the same either way;
only the narrative emphasis shifts.

Five metrics × ~8 rows is a table that fits on screen and tells a
two-part story: "BLEU and BERTScore both miss the clinical signal,
for different reasons."

`radeval_bertscore` is **not** in the gallery column set — it's a
radiology-tuned BERTScore variant, and including it would muddy the
"lexical → semantic → clinical" axis the gallery is organized
around. It stays in the speed table.
...
**Framing — the RL angle.** This isn't "here are radiology metrics
that disagree on the same pair" (that's a general evaluation story,
well covered elsewhere). It's: **"given a candidate rollout from a
radiology report-generation model, which reward would push the
model *toward* or *away from* that rollout?"** The pairs represent
plausible candidate completions a GRPO loop might produce against a
given reference. The scores are the reward signals each candidate
metric would emit.

Concretely, the `hyp` column is a "candidate rollout" (same thing a
GRPOTrainer would pass to a reward function); each metric column
shows what that metric would return as the per-sample reward. The
narrative column reads as "if you were training with metric X as
the reward, this rollout would score ... — which means the policy
would be pushed in direction Y."
...
Candidate rollouts (rough sketch — finalize during implementation).
Each row frames the RL consequence of picking one reward over
another. Narrative is written in the form: *"Rollout X. Under BLEU,
policy would be pushed toward/away from this. Under RadCliQ, ..."*

1. **Rollout: a clinically correct paraphrase.** ref: "Mild
   cardiomegaly." hyp: "The cardiac silhouette is mildly enlarged."
   Expected signals: BLEU ≈ 0 → under BLEU reward the policy is
   *pushed away from* a clinically correct rewording. BERTScore
   should score reasonably high → BERTScore handles paraphrase
   fine here. Clinical metrics also high → `f1chexbert` / `radcliq`
   reward the policy toward it. **Story: BLEU fails, BERTScore
   and clinical metrics agree.** (This row is the case where BLEU
   is the right foil, not BERTScore.)
2. **Rollout: flipped negation.** ref: "No pleural effusion." hyp:
   "Pleural effusion." Expected signals: BLEU moderate (3-of-4
   tokens overlap). **BERTScore very high** (dense token +
   embedding overlap; one word differs). F1CheXbert / RadGraph → 0.
   **Story: BLEU mildly wrong, BERTScore *strongly* wrong, clinical
   metrics correct. This is the headline row** — even the strongest
   drop-in NLP reward would push GRPO toward a clinically incorrect
   rollout. Using BERTScore as an RL reward in radiology is the
   danger readers likely haven't considered; BLEU-as-wrong is
   familiar.
3. **Rollout: missing finding.** ref: "Mild cardiomegaly. Small
   bilateral pleural effusions." hyp: "Mild cardiomegaly."
   Expected: BLEU moderate; BERTScore moderate-high (most tokens
   overlap); RadGraph and F1CheXbert lower (the second finding is
   missing). BERTScore under-penalizes omissions; clinical metrics
   reflect them. Training on BERTScore could let the policy learn
   that partial reports are fine.
4. **Rollout: hallucinated extra finding.** ref: "No acute
   findings." hyp: "No acute findings. Small right pneumothorax."
   BLEU / BERTScore both moderate-high (prefix matches); clinical
   metrics penalize. A BLEU- or BERTScore-trained policy could
   drift toward adding finding-like text; clinical-metric-trained
   policy would not.
5. **Rollout: exact match.** ref == hyp. All metrics score maximum.
   Baseline sanity row.
6. **Rollout: total mismatch.** No overlap, no clinical content
   shared. All metrics bottom out. Second baseline sanity row.
7-9. Vary severity / wording differences to fill out the table
   (finalize during implementation — ideally include at least one
   row where BERTScore clearly overrewards a clinically-wrong
   rollout beyond the negation case, so the headline has robustness).

**Measurement:** for each pair, run each of the 5 gallery metrics in
`per_sample=True` mode and record the single scalar — this *is* the
reward the corresponding `make_reward_fn` would emit for that
rollout.
...
## What reward this rollout would receive

Same setup everyone's is imagining: a GRPO trainer samples a
candidate completion, the reward function scores it, the score goes
back as the training signal. Table rows are plausible candidate
rollouts; columns are the reward each metric would emit.

<divergence gallery — 8-10 rows: reference | candidate rollout |
narrative | reward-as-metric columns (BLEU, RadEval-BERTScore,
F1CheXbert, RadGraph-partial, RadCliQ)>

*(Metric values below are rewards, not evaluation scores. Signs
follow each metric's native convention: ↑ = higher reward is better
for everything except `radcliq` where ↓ is better — see the
reward-direction note and the `score_transform=lambda x: -x` recipe
below for using `radcliq` as a maximize-reward signal.)*

> **Note on method.** This gallery is hand-picked to illustrate that
> the training signal changes dramatically with metric choice on
> plausible radiology rollouts. It is not a representative sample,
> not a randomized comparison, and not evidence of average metric
> superiority — examples were selected specifically to show
> consequential disagreement. Treat the rows as existence proofs
> ("metric choice *can* matter in this way") not prevalence
> estimates. For population-level comparisons, see the RadEval paper
> and the
> RadCliQ reference studies.

**Three rollouts worth zooming in on** (each framed as the training
consequence of the reward choice, not just the score disagreement).
The headline is **row 2** — if readers skim only one row, this is
the one.

1. *Rollout: clinically correct paraphrase* (row X). BLEU ≈ 0 →
   under a BLEU reward, GRPO would push the policy **away** from
   generating this rewording. BERTScore and F1CheXbert both high,
   RadCliQ low (≈ 0.08; lower = better) → under any *non-BLEU*
   reward, GRPO would push the policy **toward** the same rollout.
   Same completion, opposite gradient under the two rewards.
2. *Rollout: flipped negation* (row Y) — **headline**. ref: "No
   pleural effusion." hyp: "Pleural effusion." BLEU moderate; **BERTScore
   high** (most tokens + embeddings still overlap); F1CheXbert 0.0
   and RadCliQ high (≈ 0.9; remember ↓ = better). Under a
   **BERTScore reward, GRPO would partially reward flipping a
   negation** — corrupting the clinical meaning while keeping the
   text "looking right." Clinical rewards correctly score 0/high.
   **Even the strongest drop-in NLP reward is actively dangerous
   here**; clinical metrics are not interchangeable with semantic
   ones for radiology RL.
3. *Rollout: missing finding* (row Z). BLEU and BERTScore both
   moderate-high (the present finding is still there, matching in
   text and embedding); RadGraph and F1CheXbert lower. Neither
   lexical nor semantic reward penalizes omission strongly;
   RadGraph does. Trains "pass if partially correct" vs "complete
   the finding list."
...
- Speed table covers each metric in the inclusion set (per the
  ...
  cost.
- Divergence gallery includes at least one row where a **strong
  NLP baseline (BERTScore)** would *reward* a clinically-wrong
  rollout (negation flip) that clinical metrics correctly
  penalize — the "even BERTScore isn't enough" moment. BLEU is
  also in the gallery columns as a familiar lexical reference
  point.
...
- On hand-picked plausible rollouts, different reward metrics would
  push the GRPO policy in opposite directions — including the
  strong drop-in baseline: **BERTScore gives a negation-flipped
  rollout a high reward that clinical metrics correctly score
  near-zero.** Even the best standard-NLP reward is misleading
  here. Reward choice shapes what the model learns to produce, not
  just how we score it afterwards. (Prevalence not estimated;
  see the RadEval paper for population-level comparisons.)
...
- **BERTScore on a negation flip is high** (strong headline). On
  "No pleural effusion." → "Pleural effusion.", BERTScore's dense
  token + embedding overlap should produce a score that's clearly
  high (the headline claim rests on this), while F1CheXbert and
  RadGraph score 0. If measured BERTScore here is already low, the
  "even BERTScore misleads RL" story weakens and we fall back to
  BLEU-as-headline.
- RadCliQ will score the negation-flip case worse (higher
  distance) than a paraphrase. Confirmed in spirit by the existing
  test-suite pattern but not yet measured on our curated set.
- BLEU's score on clinically-critical negation flips will be
  non-zero due to substantial lexical overlap — makes BLEU a
  secondary (not headline) foil.

If E2 runs and these hypotheses don't hold on the chosen pairs, the
curation (and potentially the headline baseline choice) is wrong
and we swap accordingly. The script's output is the data; the doc's
narrative is shaped to match.
...
- On hand-picked plausible rollouts, different reward metrics
  would push the policy in opposite directions — including the
  strong drop-in semantic baseline: BERTScore rewards a negation
  flip that clinical metrics correctly penalize. Reward choice is
  not a formality; it's what the model learns from.

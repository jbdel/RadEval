...
1. **`make_reward_fn` hardening**
   ...
   - **`validate_rewards(values, metric_name)` helper (~15 LOC)**:
     normalizes via `v.item() if hasattr(v, "item") else v` then casts
     through `float()`; raises on NaN/Inf naming metric and sample index.
     Uniform across numpy scalars, 0-D / 1-element-1-D torch tensors,
     and Python floats.

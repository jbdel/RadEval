...
Hugging Face TRL for GRPO-style radiology-report RL. Revised through four
...
1. **Hardened `make_reward_fn`**
   ...
   - Handles conversational completions defensively: checks
     ...
     operates on scores). On first use per process, logs one
     `logging.info` line naming the heuristic.
   ...
   - Calls a tiny `validate_rewards()` helper once per batch to raise on
     NaN/Inf with metric name and offending sample index. The helper casts
     each value through `float()` first, so it catches `numpy.nan`, torch
     scalars, and Python floats uniformly. No silent defaulting.
...
1. **`make_reward_fn` hardening**
   ...
   - Defensive conversational-completion handling:
     ...
     layout); logs a one-time `logging.info` on first use;
     any other shape → raise `TypeError` with the observed shape printed.
   - `validate_rewards(values, metric_name)` helper (~15 LOC): casts each
     value through `float()` then raises on NaN/Inf, naming metric and
     sample index. Uniform across numpy / torch / Python scalars.
...
```python
_HEURISTIC_LOGGED = False

def _as_strings(completions):
    global _HEURISTIC_LOGGED
    if not completions:
        return []
    first = completions[0]
    ...
    if isinstance(first, list) and first and isinstance(first[0], dict):
        if not _HEURISTIC_LOGGED:
            logging.info(
                "make_reward_fn: extracting 'content' from the last "
                "assistant message in list[dict] completions. If your "
                "dataset uses a different layout, preprocess upstream."
            )
            _HEURISTIC_LOGGED = True
...
- **Conversational-handler heuristic may mis-score exotic message
  ...
  shapes we don't recognize); unit test covers the OpenAI-style case;
  one-time `logging.info` on first use; users on other layouts preprocess
  the completion text upstream in their dataset/collator pipeline (not
  via `score_transform`, which operates on scores).
...
> drop-in callable tested against `GRPOTrainer`, with the same interface
> expected to work for other TRL trainers that consume a reward-function
> callable (e.g., RLOO) and unchanged across LLM and VLM report
> generation. Lightweight metrics (BLEU, ROUGE, BERTScore, RadGraph,
...
| `validate_rewards` casts to `float()` before NaN/Inf check | `math.isnan` on raw value | Gemini round 3: metrics may return `numpy.nan` or tensor scalars; `float()` cast normalizes. |
| `logging.info` on first conversational-heuristic use | Silent | Gemini round 3: quiet by default, visible when users enable INFO logging. Keeps surface small. |

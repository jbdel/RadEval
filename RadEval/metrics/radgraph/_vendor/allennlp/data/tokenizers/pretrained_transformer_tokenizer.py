...
class PretrainedTransformerTokenizer(Tokenizer):
    ...
    def _reverse_engineer_special_tokens(
        ...
        # Upstream used `encode_plus(...)` which was renamed to the single
        # `__call__` method in transformers v5. On tokenizers that don't
        # emit token_type_ids (e.g. ModernBERT), we fall back to all-zeros
        # so the reverse-engineer loop still works; this is safe because
        # radgraph's models that use such tokenizers never actually consume
        # token_type_ids downstream.
        ...
        if "token_type_ids" not in dummy_output or dummy_output["token_type_ids"] is None:
    ...
    def tokenize(self, text: str) -> List[Token]:
        ...
        """
        # --- RADEVAL VENDOR PATCH (transformers v5 compatibility) ---
        # encode_plus(...) was renamed to __call__ in transformers v5. On
        # tokenizers that don't emit token_type_ids (e.g. ModernBERT), fall
        # back to zeros.
        ...
        if "token_type_ids" not in encoded_tokens or encoded_tokens["token_type_ids"] is None:
            encoded_tokens["token_type_ids"] = [0] * len(encoded_tokens["input_ids"])
        # --- END RADEVAL VENDOR PATCH ---
    ...
    def _intra_word_tokenize(
        ...
        for token_string in string_tokens:
            # --- RADEVAL VENDOR PATCH: encode_plus -> __call__ (transformers v5) ---

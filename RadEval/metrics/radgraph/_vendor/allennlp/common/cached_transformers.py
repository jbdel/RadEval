...
def get_tokenizer(model_name: str, **kwargs) -> transformers.PreTrainedTokenizer:
    # --- RADEVAL VENDOR PATCH (transformers >=4.44 compat, issue #5) ---
    # In transformers >=4.44 the tokenizer base classes grew an
    # `add_special_tokens` method whose name now collides with the legacy
    # `add_special_tokens=<bool>` kwarg some AllenNLP configs pass through.
    # Strip it here (and any other kwarg with the same name) before forwarding
    # to `AutoTokenizer.from_pretrained`. AllenNLP owns the add-special-tokens
    # behaviour at a higher level (PretrainedTransformerTokenizer) anyway, so
    # dropping it here is safe. Matches the fix in
    # https://github.com/corentin-ryr/radgraph (PR #4 upstream).

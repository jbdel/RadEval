...
class RadEval():
    def __init__(self,
        ...
        if self.do_f1hopprchexbert:
            try:
                ...
                warnings.warn(
                    f"F1HopprCheXbert unavailable ({e}); disabling do_f1hopprchexbert.")
                self.do_f1hopprchexbert = False

        if self.do_f1hopprchexbert_ct:
            try:
                from .metrics.f1hopprchexbert_ct import HopprF1CheXbertCT
                if HopprF1CheXbertCT is None:
                    raise ImportError("HopprF1CheXbertCT is not available")
                self.f1hopprchexbert_ct_scorer = HopprF1CheXbertCT()
            except (ImportError, FileNotFoundError, OSError) as e:
                warnings.warn(
        ...
        if self.do_f1hopprchexbert:
            self.metric_keys.extend([
                "f1hopprchexbert_5_micro_f1",
                "f1hopprchexbert_all_micro_f1",
                "f1hopprchexbert_5_macro_f1",
                "f1hopprchexbert_all_macro_f1"
            ])
        if self.do_f1hopprchexbert_ct:
            self.metric_keys.extend([
                "f1hopprchexbert_ct_accuracy",
                "f1hopprchexbert_ct_micro_f1",
    ...
    def compute_scores(self, refs, hyps):
        ...
        with self._make_progress() as progress:
            ...
            if self.do_f1hopprchexbert:
                self._score_chexbert(
                    scores, "f1hopprchexbert", self.f1hopprchexbert_scorer,
                    hyps, refs, n_samples, progress, metric_task)

            # ----------------------------------------------------------
            if self.do_f1hopprchexbert_ct:
                progress.update(metric_task, description="Computing HopprF1CheXbertCT")
                n_batches = math.ceil(n_samples / self.f1hopprchexbert_ct_scorer.batch_size) * 2
                batch_task = progress.add_task("  [dim]Batches", total=n_batches)
                ct_accuracy, ct_sample_acc, ct_report = self.f1hopprchexbert_ct_scorer(
                    hyps, refs, on_batch_done=lambda: progress.advance(batch_task))
                progress.remove_task(batch_task)
                if self.do_details:
                    ct_labels = {
                    ...
                    scores["f1hopprchexbert_ct"] = {
                        "f1hopprchexbert_ct_accuracy": ct_accuracy,
                        "f1hopprchexbert_ct_micro_f1": ct_report["micro avg"]["f1-score"],
                        "f1hopprchexbert_ct_macro_f1": ct_report["macro avg"]["f1-score"],
                        "f1hopprchexbert_ct_weighted_f1": ct_report["weighted avg"]["f1-score"],
                        "sample_scores": {"all_labels": ct_sample_acc},
                        "label_scores_f1": ct_labels,
                    }
                elif self.do_per_sample:
                    scores["f1hopprchexbert_ct_sample_acc"] = (
                        ct_sample_acc.tolist()
                        if hasattr(ct_sample_acc, 'tolist')
                        else list(ct_sample_acc))
                else:
                    scores["f1hopprchexbert_ct_accuracy"] = round(ct_accuracy, 4)
                    scores["f1hopprchexbert_ct_micro_f1"] = round(
                        ct_report["micro avg"]["f1-score"], 4)
                    scores["f1hopprchexbert_ct_macro_f1"] = round(

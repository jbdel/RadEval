...
class PretrainedTransformerIndexer(TokenIndexer):
    ...
    def __init__(
        ...
        # --- RADEVAL VENDOR PATCH (transformers v5 compatibility) ---
        # Cache the single-sequence special-token ids once at init time, so the
        # hot path in `_postprocess_output` can prepend/append them directly
        # instead of calling `tokenizer.build_inputs_with_special_tokens(...)`.
        # The latter was removed from TokenizersBackend (fast tokenizers) in
        # transformers v5. The AllenNLP-side reverse-engineer already gave us
        # the ids we need on both ends of a single sequence.
        self._single_prefix_ids = [
            t.text_id for t in self._allennlp_tokenizer.single_sequence_start_tokens
        ]
        self._single_suffix_ids = [
            t.text_id for t in self._allennlp_tokenizer.single_sequence_end_tokens
        ]
        # --- END RADEVAL VENDOR PATCH ---
    ...
    def _postprocess_output(self, output: IndexedTokenList) -> IndexedTokenList:
        ...
        if self._max_length is not None:
            ...
            # --- RADEVAL VENDOR PATCH (transformers v5 compatibility) ---
            # Upstream used `tokenizer.build_inputs_with_special_tokens(segment)`
            # which was removed from TokenizersBackend (fast tokenizers) in
            # transformers v5. We use the cached prefix/suffix ids captured at
            # __init__ time. This is equivalent for single-sequence inputs,
            # which is the only case radgraph uses (DyGIE never calls this
            # indexer in sequence-pair mode).
            folded_indices = [
                self._single_prefix_ids + list(segment) + self._single_suffix_ids

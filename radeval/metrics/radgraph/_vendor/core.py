...
# We pin the cache-dir suffix to the upstream radgraph release whose
# `.tar.gz` archives we are compatible with. Deliberately kept as a
# literal so (a) RadEval users who already have weights under
# ~/.cache/radgraph/0.1.18/ don't re-download them, and (b) we don't
# depend on a `radgraph` pip distribution being installed.
_UPSTREAM_RADGRAPH_VERSION = "0.1.18"
CACHE_DIR = os.path.join(user_cache_dir("radgraph"), _UPSTREAM_RADGRAPH_VERSION)
